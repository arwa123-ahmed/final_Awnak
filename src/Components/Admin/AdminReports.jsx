...
const AdminReports = () => {
  ...
  const [actionConfig, setActionConfig] = useState(null); // للتحكم في مودال التأكيد
  const [processingId, setProcessingId] = useState(null); // لإظهار لودر على الزرار نفسه
  ...
  const handleToggleAction = async () => {
    ...
    const { userId } = actionConfig;

    setProcessingId(userId);
    setActionConfig(null); // إغلاق مودال التأكيد

    try {
      const res = await fetch(
        `http://127.0.0.1:8000/api/admin/users/${userId}/suspend`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] p-6 lg:p-10 font-sans" dir="ltr">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="grid gap-6">
          {reports.length === 0 ? (
            ...
            reports.map((report) => {
              const targetUser = report.reported_user || report.reportedUser;
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2">
                            <span
                              className={`h-1.5 w-1.5 rounded-full ${targetUser?.is_suspended ? "bg-red-500 animate-pulse" : "bg-green-500"}`}
                            ></span>
                            <p className="text-[10px] font-bold text-gray-400 uppercase">
      ...
      {/* ─── Action Confirmation Modal (THE NEW UI) ─── */}
      {actionConfig && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-gray-900/40 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm overflow-hidden rounded-[2.5rem] bg-white shadow-2xl animate-in fade-in zoom-in duration-200">
            <div
              className={`p-8 text-center ${actionConfig.type === "suspend" ? "bg-red-50" : "bg-emerald-50"}`}
            >
              <div
                className={`mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-3xl ${actionConfig.type === "suspend" ? "bg-red-100 text-red-600" : "bg-emerald-100 text-emerald-600"}`}
              >
                {actionConfig.type === "suspend" ? (
                  <UserMinus size={32} />
                ) : (
                  <UserCheck size={32} />
                )}
              ...
              <p className="mt-2 text-sm text-gray-500 font-medium leading-relaxed">
                ...
                This will affect their platform access immediately.
              </p>
            </div>
            <div className="flex gap-3 p-6 bg-white">
              <button
                onClick={() => setActionConfig(null)}
      ...
      {/* ─── Compact Evidence Modal ─── */}
      {selectedChat && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-[2px] p-4 transition-all">
          <div className="flex w-full max-w-md flex-col overflow-hidden rounded-[2.5rem] bg-white shadow-2xl animate-in slide-in-from-bottom duration-300 h-[75vh]">
            <header className="flex items-center justify-between border-b border-gray-50 p-6">
              <div className="flex items-center gap-3">
                <div className="h-10 w-10 rounded-2xl bg-gray-900 flex items-center justify-center text-white shadow-lg shadow-gray-200">
                  <MessageSquare size={18} />
                </div>
                <div>
                  <h3 className="text-[11px] font-black uppercase tracking-[0.1em] text-gray-900">
                    Evidence Logs
                  </h3>
            ...
            <div className="flex-1 overflow-y-auto bg-[#fafafa] p-6 space-y-4">
              {selectedChat.messages.length > 0 ? (
                selectedChat.messages.map((msg) => {
                  const isReporter =
                    String(msg.sender_id) === String(selectedChat.reporter?.id);
                  return (
                    <div
                      key={msg.id}
                      className={`flex ${isReporter ? "justify-start" : "justify-end"}`}
                    >
                      <div
                        className={`max-w-[85%] rounded-[1.5rem] px-4 py-3 text-[13px] shadow-sm ${
                          isReporter
                      ...
                      >
                        <div className="mb-1.5 flex items-center justify-between gap-6">
                          <span
                            className={`text-[8px] font-black uppercase ${isReporter ? "text-blue-500" : "text-gray-500"}`}
                          >
