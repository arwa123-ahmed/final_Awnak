    ...
    <div className="bg-white dark:!bg-slate-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-md p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="flex items-center border-b border-green-200 dark:border-green-800 pb-3 gap-2">
          ...
          <span className="dark:text-gray-200">{personalInfo?.name || "-"}</span>
        </div>
        <div className="flex items-center border-b border-green-200 dark:border-green-800 pb-3 gap-2">
          <FaIdCard className="text-green-600" />
          <span className="font-medium text-gray-700 dark:text-gray-300">{t("nationalId")}:</span>
          <span className="dark:text-gray-200">{personalInfo?.national_id || "-"}</span>
        </div>
        <div className="flex items-center border-b border-green-200 dark:border-green-800 pb-3 gap-2">
          <FaCalendarAlt className="text-green-600" />
          <span className="font-medium text-gray-700 dark:text-gray-300">{t("gender")}:</span>
          <span className="dark:text-gray-200">{personalInfo?.gender || "-"}</span>
        </div>
        <div className="border-b border-green-200 dark:border-green-800 pb-4 md:col-span-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 ml-6">
            <div><p className="text-xs text-gray-500 dark:text-gray-400">{t("country")}:</p><span className="dark:text-gray-200">{personalInfo?.nationality || "-"}</span></div>
            <div><p className="text-xs text-gray-500 dark:text-gray-400">{t("city")}:</p><span className="dark:text-gray-200">{personalInfo?.city || "-"}</span></div>
            <div><p className="text-xs text-gray-500 dark:text-gray-400">{t("area")}:</p><span className="dark:text-gray-200">{personalInfo?.street || "-"}</span></div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <FaPhone className="text-green-600" />
          <span className="font-medium text-gray-700 dark:text-gray-300">{t("phone")}:</span>
          <span className="dark:text-gray-200">{personalInfo?.phone || "-"}</span>
        </div>
        <div className="flex items-center gap-2">
          <FaEnvelope className="text-green-600" />
          <span className="font-medium text-gray-700 dark:text-gray-300">{t("email")}:</span>
          <span className="dark:text-gray-200">{personalInfo?.email || "-"}</span>
        </div>
      </div>
    </div>
  );
    ...
    >
      <div className="flex justify-between items-start">
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 line-clamp-2 mb-2">{service.description}</p>
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <span className={`text-xs px-2 py-1 rounded-full font-medium ${isPending ? "bg-amber-50 text-amber-600" : "bg-blue-50 text-blue-600"}`}>
              {service.status}
            </span>
        ...
        <div className="flex flex-col gap-2 ml-3" onClick={(e) => e.stopPropagation()}>
          {isPending && (
            <button onClick={() => onEdit(service)} className="p-2 rounded-lg bg-green-50 dark:bg-green-900/30 hover:bg-green-100 text-green-600 transition-colors" title="Edit">
              <FaEdit size={14} />
            </button>
          )}
          <button onClick={() => onDelete(service.id)} className="p-2 rounded-lg bg-red-50 dark:bg-red-900/30 hover:bg-red-100 text-red-500 transition-colors" title="Delete">
            <FaTrash size={14} />
          </button>
        </div>
      </div>
  ...
  return (
    <div className="bg-white dark:!bg-slate-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-md p-6">
      ...
      {loading ? (
        ...
        <p className="text-gray-400 text-center py-8">{t("noRequestedServices")}</p>
      ) : (
        <div className="space-y-3">
          {services.map((service) => (
            <ProfileServiceCard key={service.id} service={service} onEdit={onEdit} onDelete={onDelete} onNavigate={onNavigate} />
          ))}
        </div>
      )}
    </div>
...
const VolunteeringTab = ({ services, onEdit, onDelete, onNavigate, loading }) => {
  ...
  return (
    <div className="bg-white dark:!bg-slate-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-md p-6">
      ...
      <div className="mb-6">
        <h3 className="text-lg font-semibold text-gray-800 dark:text-white mb-2">{t("progressLevel")}</h3>
        <div className="flex items-center gap-4">
          <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-4 overflow-hidden">
            <div className={`h-4 ${levelInfo.progressColor}`} style={{
              width: totalMinutes === 0 ? "0%" : totalMinutes < 200 ? `${(totalMinutes / 200) * 100}%` : "100%",
            }} />
          </div>
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 mt-2">{totalMinutes} min – {t("level")}: {levelInfo.level}</p>
      </div>
      {loading ? (
        <p className="text-gray-400 text-center py-8">{t("loading")}</p>
      ) : services.length === 0 ? (
        <p className="text-gray-400 text-center py-8">{t("noVolunteeringServices")}</p>
      ) : (
        <div className="space-y-3">
          {services.map((service) => (
            <ProfileServiceCard key={service.id} service={service} onEdit={onEdit} onDelete={onDelete} onNavigate={onNavigate} />
...
const EditServiceModal = ({ service, categories, onClose, onSave }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await axios.put(`http://72.62.186.133/api/services/${service.id}`, form, { headers: { Authorization: `Bearer ${token}` } });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
      <div className="bg-white dark:!bg-slate-800 rounded-3xl shadow-2xl p-7 w-[95%] max-w-md flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-bold text-gray-800 dark:text-white">{t("editService")}</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-2xl">×</button>
        </div>
        <div className="h-px bg-gray-100 dark:bg-gray-700" />
        <div className="flex flex-col gap-1">
          <label className="text-sm font-semibold text-gray-600 dark:text-gray-300">{t("description")}</label>
        ...
        <div className="flex flex-col gap-1">
          <label className="text-sm font-semibold text-gray-600 dark:text-gray-300">{t("timeMin")}</label>
          <input type="number" value={form.timesalary} onChange={(e) => setForm((p) => ({ ...p, timesalary: e.target.value }))}
        ...
        <div className="flex flex-col gap-1">
          <label className="text-sm font-semibold text-gray-600 dark:text-gray-300">{t("category")}</label>
        ...
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl border-2 border-gray-200 dark:border-gray-600 text-gray-500 dark:text-gray-400 font-semibold text-sm hover:bg-gray-50 dark:hover:bg-gray-700">
            {t("cancel")}
          </button>
          <button onClick={handleSave} disabled={saving} className="flex-1 py-2.5 rounded-xl bg-green-300 hover:bg-green-400 text-white font-bold text-sm disabled:opacity-60">
...
export default function Account() {
  ...
  useEffect(() => {
    ...
    }).catch(console.error).finally(() => setServicesLoading(false));
  }, [user]);
  ...
  const handleNavigateToService = (service) => {
    if (service.type === "offer") navigate(`/customer/category/${service.category_id}`);
  ...
  const handleDelete = async (serviceId) => {
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.delete(`http://72.62.186.133/api/services/${serviceId}`, { headers: { Authorization: `Bearer ${token}` } });
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case "personal": return <PersonalInfoTab personalInfo={user} />;
      case "requested": return <RequestedServicesTab services={myRequests} onEdit={setEditingService} onDelete={handleDelete} onNavigate={handleNavigateToService} loading={servicesLoading} />;
      case "volunteering": return <VolunteeringTab services={myOffers} onEdit={setEditingService} onDelete={handleDelete} onNavigate={handleNavigateToService} loading={servicesLoading} />;
      default: return null;
    }
  };
    ...
    <div className="min-h-screen p-4 md:p-8 dark:!bg-gray-900">
      <div className="max-w-4xl mx-auto bg-white dark:!bg-slate-800 rounded-lg shadow-lg p-6 md:p-8">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-green-600">{t("myProfile")}</h1>
        </div>
        ...
        <div className="flex flex-col items-center mb-8">
          <div className={`w-28 h-28 rounded-full border-4 shadow-lg overflow-hidden flex items-center justify-center ${levelInfo.levelColor}`}>
            <img key={profileImage} src={profileImage} alt="avatar" className="w-full h-full object-cover"
          ...
          <p className="mt-3 text-sm font-semibold text-gray-700 dark:text-gray-300">
            {t("level")}: <span className="text-green-600">{levelInfo.level}</span>
          </p>
        </div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between mt-6">
          ...
          >
            <span className="text-lg font-semibold text-green-700 dark:text-green-400 mr-2">{t("currentBalance")}</span>
            <span className="text-2xl font-bold text-green-700 dark:text-green-400">{user?.balance ?? 0}</span>
          </div>
      ...
      {editingService && (
        <EditServiceModal service={editingService} categories={categories} onClose={() => setEditingService(null)} onSave={handleSaveEdit} />
      )}
    </div>
  );
}